...
void setup()
{
  ...
  delay(1000);
  dorji_setfreq(0, transmit_frequency, receive_frequency, dorji); // tx rx
  dorji_readback(dorji);
...
void messageExample() {
  ...
  char *message = "Hi there! This is a message from receiver.";
  APRS_sendMsg(message, strlen(message));

  //-----------------------------------------------------
  //char my_message[10] = "";
  //ltoa(millis(), my_message, DEC);
  //APRS_sendMsg(my_message, strlen(my_message));
  //-----------------------------------------------------
...
void processPacket() {
  if (gotPacket) {
    ...
    for (int i = 0; i < incomingPacket.len; i++) {
      ...
      my_data[i]=incomingPacket.info[i];
    }
    Serial.println("");
    //---------------------------------------------------
    //for (int i = 0; i < incomingPacket.len; i++) {
    ...
    //Serial.println(memcmp(my_data, ":AA3BBB-0 :GET MILLIS TIME", 26));
    if(memcmp(my_data, ":AA3BBB-0 :Hi there! This is a message.", 39)==0){
      //char my_message[10] = "";
      //ltoa(millis(), my_message, DEC);
